...
def get_row_col_encoding():
    row_col_encoding = np.arange(1, NUMBER_OF_FLASHES+1, dtype=np.int8)
    return row_col_encoding


row_col_encoding = get_row_col_encoding()
...
print(row_col_order)
test_data = np.repeat(test_data, number_of_trials * (NUMBER_OF_FLASHES) * 2)
test_data = np.append(test_data, "")
epoch_numbers = np.repeat(epoch_numbers, number_of_trials * (NUMBER_OF_FLASHES) * 2)
...
def create_time_for_one_epoch():
    ...
    one_trial = np.tile([intensification_time, darken_time], NUMBER_OF_FLASHES)
...
n = (NUMBER_OF_FLASHES) * number_of_trials * number_of_epochs * 2 + 1
# darkened_color = "#373737"
darkened_color = "#1A1A1A"

color_order = np.full(n, darkened_color)
intensified_colors = ["#FFFFFF",'#FF0000']
total_intensified_colors = np.random.choice(intensified_colors, (NUMBER_OF_FLASHES) * number_of_trials * number_of_epochs,
                                            replace=True)
color_order[1::2] = total_intensified_colors

fonts = np.append(
    np.tile(np.array([darkened_font_size, intensified_font_size]),
            (NUMBER_OF_FLASHES) * number_of_trials * number_of_epochs),
    darkened_font_size)
...
class Gui:
    ...
    def simulate(self):
        ...
        self.Label_Matrix[int(line/3), int(line%3)].config(foreground=self.time_line[self.Index][4],
